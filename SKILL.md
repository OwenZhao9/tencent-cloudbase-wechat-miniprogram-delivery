...
- Ask for missing project facts before making risky assumptions. Use `references/intake-questionnaire.md` for from-scratch projects, incomplete handoffs, or ambiguous customer requirements.
- Keep intake short: ask only blocking questions first, then proceed with reasonable defaults and list assumptions.
...
1. **Intake**
   - If no code exists, read `references/intake-questionnaire.md` and ask the minimum required questions before scaffolding.
   - If code exists, inspect first and ask only for missing paths, AppID, `envId`, credentials, or business rules that cannot be inferred safely.
   - If the user wants speed, state assumptions and begin with an MVP path.
...
- Read `references/intake-questionnaire.md` when starting with no code, unclear requirements, missing CloudBase/AppID context, or a user asks for a guided setup.
